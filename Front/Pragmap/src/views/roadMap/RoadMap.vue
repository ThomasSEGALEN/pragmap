<template>
    ...
    <div>
      ...
      <VueFlow v-model="elements" class="vue-flow-basic-example" :default-zoom="1" :min-zoom="0.2" :max-zoom="4"
        ...
        <template #node-tache="nodeProps">
          <TacheNode @node-clicked="selectedNodeId = $event" v-bind="nodeProps" />
        </template>
        <template #node-jalon="nodeProps">
          <JalonNode @node-clicked="selectedNodeId = $event" v-bind="nodeProps" />
        </template>
        <template #node-livrable="nodeProps">
          <LivrableNode @node-clicked="selectedNodeId = $event" v-bind="nodeProps" />
        </template>
      </VueFlow>
    </div>
  </Layout>
  <div v-if="selectedNodeId && selectedNode" class="settingsWindow">
    <h2>Node Settings</h2>
    <div v-for="(value, key) in selectedNode.data" :key="key" v-if="selectedNode">
      <label>{{ key }}</label>
      <textarea v-if="typeof key === 'string' && key === 'description'" v-model="selectedNode.data[key]" class="description"></textarea>
...
const addNode = (type: string) => {
  ...
  const lastNode = elements.value[elements.value.length - 1] as { position: { x: number, y: number } } | undefined;
