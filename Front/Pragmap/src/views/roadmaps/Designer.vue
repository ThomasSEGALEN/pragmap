...
import { type Elements, useVueFlow, useGetPointerPosition, VueFlow, type MouseTouchEvent, type EdgeMouseEvent, type XYPosition, type ViewportTransform } from '@vue-flow/core'
...
const connectToRoadmap = async () => {
	...
	signalRService.receiveUserPosition((userPosition) => {
		const existingUserPositionIndex = userPositions.findIndex(up => up.userId === userPosition.userId);
...
const mouseMove = (event: MouseEvent) => {
	...
	roadMapMouseMoveTimeout = setTimeout(() => {
...
<template>
	<div class="w-full relative">
		...
		<VueFlow v-model="elements" class="vue-flow-basic-example" :default-zoom="1" :min-zoom="0.2" :max-zoom="4"
			style="height: 80vh" @pane-mouse-move="mouseMove" @move="handleMove">
			<div v-for="(userPosition, index) in userPositions" :key="index" class="user-cursor"
				:style="{ left: userPosition.xPosition + 'px', top: userPosition.yPosition + 'px', scale: userPosition.scale }">
				<span>{{ userPosition.username }}</span>
			</div>
			<Background pattern-color="#aaa" :gap="8" />
			<MiniMap />
			<template #node-tache="nodeProps">
				<TaskNode v-bind="nodeProps" @node-clicked="selectedNodeId = $event" />
			</template>
			<template #node-jalon="nodeProps">
				<MilestoneNode v-bind="nodeProps" @node-clicked="selectedNodeId = $event" />
			</template>
			<template #node-livrable="nodeProps">
				<DeliverableNode v-bind="nodeProps" @node-clicked="selectedNodeId = $event" />
			</template>
		</VueFlow>
